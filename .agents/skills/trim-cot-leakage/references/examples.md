...
**Fixed:** "Slash input resolves against the visible catalog — the plain-text-reference decision, owned by the note that recorded that decision."
...
Both live facts survive — the current mechanism and the standing failure behavior — restated in the present. The bug's biography belongs to the PR and its note.
...
"This cut" / "v1" / "today" go stale the moment they merge. A historical stage name inside a note's change-story section ("the first cut shipped X") is current-state-safe; the indexical form never is.
...
**Fixed (in a note's Alternatives considered):** "**Caching the resolved spec.** Rejected: the spec depends on per-call cwd, so a cache keyed by request would serve stale roots."
...
An unaided pass deleted this, reasoning that issue citations belong in notes. Wrong direction: issues resolve at HEAD from any surface, and "#N owns the follow-up" is the sanctioned home for deferred work in a README. What notes and postmortems additionally sanction is citing *merged PRs* as evidence.
...
**Keep (after fixing):** `#[allow(clippy::indexing_slicing)] // the one-element literal guarantees index 0.`
