---
name: prose-standard
description: Use when writing, reviewing, restoring, trimming, or auditing prose in this repo, including deciding where documentation or comments are required across Markdown, rustdoc/JSDoc, code and test comments, descriptions, diagnostics, and CLI strings.
---

# Prose Standard

Write enough to preserve the contract, then remove reasoning transcripts, repetition, and decoration. A contract is an obligation, invariant, precondition, postcondition, or compatibility promise that a caller, callee, implementer, producer, or consumer relies on. This skill owns editorial judgment and required prose coverage; use the [documentation standard](../../../docs/AGENTS.md) for placement and layering rules, and [trim-cot-leakage](../trim-cot-leakage/SKILL.md) for hunting and fixing reasoning-transcript leakage. It is guidance, not a script.
...
Always exclude `target/` from discovery, review, and edits, even when the requested scope is the whole repository. Do not follow a symlink into it. Put exclusions after inclusion globs so a later include cannot re-admit it: for example, end ripgrep commands with `--glob '!target/**'`, and give Git commands an explicit `:(exclude)target/**` pathspec. If the requested scope contains only `target/`, report that no eligible files remain.

Also exclude `docs/notes/archived/` and `docs/notes/rejected/` from prose review and edits. Archived and rejected notes are frozen snapshots; inspect an exact target only to understand a historical inbound citation, never to modernize its prose or outbound links.
...
- **Public API docs (rustdoc/JSDoc):** document caller-visible return distinctions, errors or panics, side effects, ownership, timing, cancellation, and durability.
...
- **Runbooks and how-tos:** include prerequisites, required actions, the real entry path, observable verification, and concise warnings.
...
- **Notes:** retain unique rationale, mechanisms, alternatives, consequences, shipped verification evidence, and named coverage gaps. Implemented notes state shipped reality in the present tense; remove planning checklists, not evidence of what pins the decision.
...
6. Run the narrow relevant checks, documentation gates, `git diff --check`, and behavior tests for visible strings. Verify the final diff contains no `target/` path and report any accidental build-artifact match rather than claiming a clean exclusion history.
