...
pub trait HttpContextExt<T> {
    ...
    fn with_http_error_context(self, function_name: &str, url: &str, error_type: &str) -> anyhow::Result<T>;
}

impl<T> HttpContextExt<T> for Result<T, reqwest::Error> {
    fn with_http_context(self, function_name: &str, url: &str) -> anyhow::Result<T> {
        self.context(create_reqwest_context(function_name, url, "HTTP_REQUEST_ERR"))
    }
    
    fn with_http_status_context(self, function_name: &str, url: &str) -> anyhow::Result<T> {
        self.context(create_reqwest_context(function_name, url, "HTTP_STATUS_ERR"))
    }
